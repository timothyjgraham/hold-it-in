...
function _createTowerSignTexture(towerType) {
    ...
    ctx.fillText(t('tower.intro.tagline'), cW / 2, barH / 2);

    // Tower name (large)
    ctx.fillStyle = hexCSS(PALETTE.ink);
    const towerName = t('tower.intro.' + towerType + '.name');
    const nameLen = towerName.length;
    const nameFontSize = nameLen > 18 ? 96 : nameLen > 14 ? 112 : 130;
    ctx.font = getCanvasFont(nameFontSize, true);
    ctx.fillText(towerName, cW / 2, barH + 130);
    ...
    for (let i = 0; i < data.traits.length; i++) {
        ctx.fillText('• ' + t('tower.intro.' + towerType + '.t' + (i + 1)), cW / 2, traitStartY + i * traitLineH);
    }
